...
`0.5.3` is the stable patch line. It preserves the `0.5.2` host API, defaults runtime assets to LuaSkills core `v0.5.3`, and includes JSON container type fidelity plus strict Windows verbatim-path boundaries.
...
The release version is stored in `VERSION`. Go users consume SDK versions through Go module tags such as `v0.5.3`.
